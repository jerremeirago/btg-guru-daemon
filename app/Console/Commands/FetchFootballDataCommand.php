...
class FetchFootballDataCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'api:football 
     ...
     * @var string
     */
    protected $description = 'Fetch football data from RapidAPI for a specific date';
    ...
    public function handle(): int
    {
        ...
        $this->info("Fetching football data for date: {$day}/{$month}/{$year}");
        ...
        try {
            // Fetch football matches for the specified date
            ...
            if (isset($matches['data']) && is_array($matches['data'])) {
                ...
                $this->info("Successfully fetched {$matchCount} football matches.");
            ...
            $this->error("Failed to fetch football data: {$e->getMessage()}");

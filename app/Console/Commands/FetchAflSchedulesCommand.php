...
class FetchAflSchedulesCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'api:afl:schedules {--recurring : Run the command in recurring mode using queue}';
     ...
     * @var string
     */
    protected $description = 'Fetch AFL schedules from GoalServe API';
    ...
    {
        $this->info('Fetching AFL schedules from GoalServe API...');
        ...
        if (empty($data['response'])) {
            $this->error('Failed to fetch AFL schedules');
            return Command::FAILURE;
        }

        $this->info('Successfully fetched AFL schedules');
        ...
        $latestData = AflApiResponse::updateOrCreate([
            ...
            'request_id' => Str::uuid(),
            'request_type' => AflRequestType::Schedules->name,
        ]);
